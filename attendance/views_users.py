...
# ==================== EXAMINATION MODULE (Teacher & Student) ====================
...
def generate_report_card(request, student_id, term_id):
    """Generate report card for a student/term (Student or Admin)."""
    # Allow student to view own report card, admin/teacher to view any
    if request.user.role == User.IS_STUDENT:
        student = request.user.student_profile
        if student.reg_number != student_id:
            return HttpResponse("Unauthorized", status=403)
    else:
    ...
    # Retrieve all exams and marks for this student in the term
    exams = Exam.objects.filter(term=term, course_unit__course=student.course)
    marks_data = []
    for exam in exams:
        mark = MarksEntry.objects.filter(student=student, exam=exam).first()
        marks_data.append({
            'exam': exam,
            'marks': mark.marks_obtained if mark else '-',
            'grade': mark.grade.name if mark and mark.grade else '-'
        })

    # Generate PDF report card (similar to transcript)
    from reportlab.pdfgen import canvas
    from io import BytesIO
    buffer = BytesIO()
    p = canvas.Canvas(buffer, pagesize=(595.27, 841.89))
    p.drawString(100, 800, f"REPORT CARD - {student.name}")
    # ... add table of marks
    p.showPage()
    p.save()
    buffer.seek(0)
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="report_card_{student.reg_number}_{term.id}.pdf"'
...
def parent_dashboard(request):
    """Parent view: see child's results, attendance, fees."""
    if request.user.role != User.IS_PARENT:
        return HttpResponse("Unauthorized", status=403)

    parent = request.user.parent_profile
    children = parent.students.all()
    # Show summary for each child
    child_data = []
    for student in children:
        ...
        # Fees
        current_term = AcademicTerm.objects.filter(is_current=True).first()
        fee_account = StudentTermFee.objects.filter(student=student, term=current_term).first()
        fee_balance = fee_account.remaining_balance if fee_account else 0
...
def manage_online_courses(request):
    ...
    if request.user.role == User.IS_TEACHER:
        teacher = request.user.teacher_profile
        courses = OnlineCourse.objects.filter(instructor=teacher)
    else:
    ...
    return render(request, 'attendance/manage_online_courses.html', {
        'courses': courses,
        'course_units': course_units,
        'teachers': teachers,
    })
...
def view_course_materials(request, course_id):
    """Student view lessons, notes."""
    if request.user.role != User.IS_STUDENT:
        return HttpResponse("Unauthorized", status=403)

    course = get_object_or_404(OnlineCourse, id=course_id, is_active=True)
    lessons = Lesson.objects.filter(course=course).order_by('order')
    notes = Note.objects.filter(course=course)
    assignments = Assignment.objects.filter(lesson__course=course)
    return render(request, 'attendance/view_course_materials.html', {
        'course': course,
        'lessons': lessons,
        'notes': notes,
...
def submit_assignment(request, assignment_id):
    ...
    if request.method == 'POST' and request.FILES.get('file'):
        ...
        if not created:
            ...
            submission.save()
            messages.info(request, "Assignment re-submitted.")
        else:
            messages.success(request, "Assignment submitted.")
        return redirect('attendance:view_course_materials', course_id=assignment.lesson.course.id)
    return redirect('attendance:view_course_materials', course_id=assignment.lesson.course.id)
...
def grade_assignment(request, submission_id):
    ...
    if request.method == 'POST':
        ...
        return redirect('attendance:manage_online_courses')
    return render(request, 'attendance/grade_assignment.html', {'submission': submission})


@login_required
def take_online_exam(request, exam_id):
    """Student takes exam (time-limited)."""
    if request.user.role != User.IS_STUDENT:
        return HttpResponse("Unauthorized", status=403)

    exam = get_object_or_404(OnlineExam, id=exam_id, is_published=True)
    student = request.user.student_profile
    # Check if student already submitted
    if OnlineExamAnswer.objects.filter(question__online_exam=exam, student=student).exists():
        messages.warning(request, "You have already taken this exam.")
        return redirect('attendance:view_course_materials', course_id=exam.online_course.id)

    questions = OnlineExamQuestion.objects.filter(online_exam=exam)
    if request.method == 'POST':
        # Process answers
    ...
    return render(request, 'attendance/take_online_exam.html', {
        'exam': exam,
        'questions': questions,
    })
...
def registrar_dashboard(request):
    ...
    pending_transfers = 0  # Placeholder – you can add a model for transfer requests later
    ...
    return render(request, 'attendance/registrar_dashboard.html', context)